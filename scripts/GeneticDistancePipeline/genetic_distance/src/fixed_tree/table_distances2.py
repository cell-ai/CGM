...
Parte do main.py (roda no final do pipeline)
    ...
    try:
        with open(file_path, "r") as f:
            for line in f:
                ...
                species1, species2, distance = parts
                species_pair = (species1, species2)

                try:
                    distances[species_pair][gene_name] = float(distance)
                except ValueError:
                    print(f"🚨 Erro ao converter distância para {species1}-{species2}: {distance}")
                    continue
...
def create_all_distances_table(directory_path, csv_file_path):
    ...
    for filename in os.listdir(directory_path):
        if filename.startswith("RAxML_distances.") and filename.endswith("_matrix"):
            ...
            update_status, message = update_csv_table(csv_file_path, distances, gene_name)           
